...
# Script for plotting encrypted retrieval performance across diseases
...
if(INCLUDE_ENCRYPTION == True):
    ...
    results_total_df = pd.read_csv('../Results/Gemini2p5flash_testSO_v2_NEC_' + model_name_abbr + '.csv')
...
positions = np.arange(1, (len(disease_list) + 1)*1, step = 1)*1.5
pos_offset = 0.15
bp_width = 0.25

# nDCG without Encryption
bplot = plt.boxplot(sum_df['NDCG_WOE'], 
            vert= True, 
            patch_artist = True, 
...
for ii in range(0,len(disease_list),1):
    med_val = sum_df.loc[ii,'MED_WOE']
    iqr_val = sum_df.loc[ii,'IQR_WOE']
    avg_val = sum_df.loc[ii,'AVG_WOE']
    std_val = sum_df.loc[ii,'STD_WOE']
    texty_val = np.percentile(sum_df.loc[ii,'NDCG_WOE'],25)-(0.02*((-1)**ii))
    # plt.text(positions[ii]-5*pos_offset, med_val-0.1, f'Med.: {med_val:.2f}', color = 'navy')
    # plt.text(positions[ii]-5*pos_offset, med_val-0.15, f'  IQR: {iqr_val:.2f}', color = 'navy')
    
    plt.text(positions[ii]-6*pos_offset, texty_val*1-0.1, f'Mean: {avg_val:.2f}', color = 'navy')
    plt.text(positions[ii]-6*pos_offset, texty_val*1-0.15, f'Std.: {std_val:.2f}', color = 'navy')


# nDCG with Encryption
bplot = plt.boxplot(sum_df['NDCG_WE'], 
            vert= True, 
            patch_artist = True, 
            positions = positions+pos_offset,
            widths = bp_width,
...
for ii in range(0,len(disease_list),1):
    med_val = sum_df.loc[ii,'MED_WE']
    iqr_val = sum_df.loc[ii,'IQR_WE']
    avg_val = sum_df.loc[ii,'AVG_WE']
    std_val = sum_df.loc[ii,'STD_WE']
    texty_val = np.percentile(sum_df.loc[ii,'NDCG_WE'],25)-(0.02*((-1)**ii))
    wstext_y =  upper_whisker(np.array(sum_df.loc[ii, 'NDCG_WE'])) #np.percentile(sum_df.loc[ii,'NDCG_WE'],95)
    
    # plt.text(positions[ii]+pos_offset, med_val-0, f'Med.: {med_val:.2f}', color = 'maroon')
    # plt.text(positions[ii]+pos_offset, med_val-0.05, f'  IQR: {iqr_val:.2f}', color = 'maroon')

    plt.text(positions[ii]+1*pos_offset, texty_val*1-0.1, f'Mean: {avg_val:.2f}', color = 'maroon')
    plt.text(positions[ii]+1*pos_offset, texty_val*1-0.15, f' Std.: {std_val:.2f}', color = 'maroon')


    # add_compline(plt, positions[ii]-pos_offset, positions[ii]+pos_offset, wstext_y, 0.05,
                 # text = 'WD: ' + "{:.2f}".format(sum_df.loc[ii, 'WS_DIST']))
    
    add_compline(plt, positions[ii]-pos_offset, positions[ii]+pos_offset, wstext_y, 0.05,
                 text = (r"$\tau$: {:.2f}" "\n" r"$p$ {}").format(sum_df.loc[ii, 'KT_STAT'],
                                                                   format_p_scientific(sum_df.loc[ii, 'KT_PVALUE'])))
...
plt.ylim([0,1.18])
plt.grid()
# plt.title('Retrieval with and without encryption')

UObj.folder_check('Plots/')
plt.savefig('Plots/Fig1_retrieval_performance.jpg')
plt.savefig('Plots/Fig1_retrieval_performance.svg')  
#-----------------------------------------------------------------------------#        
